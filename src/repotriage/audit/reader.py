...
from repotriage.dataset.models import NormalizedIssue, ProcessedManifest
...
def iter_issues(jsonl_path: Path) -> Iterator[NormalizedIssue]:
    ...
    each raise :class:`AuditReadError` carrying the dataset path and one-based line
    number. Records are never silently skipped.
    """
    try:
        handle = jsonl_path.open("r", encoding="utf-8")
    except OSError as exc:
        raise AuditReadError(f"Unable to read dataset output {jsonl_path}: {exc}") from exc
...
def read_dataset_issues(
    ...
    :class:`AuditReadError`. The check runs after the stream is exhausted, so callers
